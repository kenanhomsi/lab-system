...
/**
 * Careers page hero with gradient overlay, CTAs, and quick highlights.
 */
export async function CareersHeroSection() {
  const t = await getTranslations("careers.hero");
  const locale = await getLocale();
  const arrow = locale === "ar" ? "arrow_downward" : "arrow_downward";

  const highlights = [
    { icon: "verified", key: "highlight1" },
    { icon: "groups", key: "highlight2" },
    { icon: "trending_up", key: "highlight3" },
  ] as const;
  ...
  return (
    <section className="relative isolate overflow-hidden">
      ...
      <div className="absolute inset-0 z-0">
        <div className="absolute -end-32 top-0 h-[480px] w-[480px] rounded-full bg-primary/[0.08] blur-[80px]" />
        <div className="absolute -start-24 bottom-0 h-[360px] w-[360px] rounded-full bg-tertiary/[0.06] blur-[80px]" />
      </div>
      ...
      <div className="relative z-10 content-container w-full py-16 md:py-24 lg:py-28">
        <div className="max-w-3xl">
          ...
          >
            <Icon name="work" filled size="sm" className="text-primary" />
          ...
          >
            ...
            >
              <Icon name="work_history" size="sm" />
              {t("primaryCta")}
              <Icon
                name={arrow}
                size="sm"
