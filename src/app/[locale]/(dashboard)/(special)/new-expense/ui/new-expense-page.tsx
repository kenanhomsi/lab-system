...
export function NewExpensePage() {
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        const res = await fetch("/api/special/lookup/expense-types");
  ...
  async function handleSubmit(e: FormEvent<HTMLFormElement>) {
    ...
    try {
      const res = await fetch("/api/special/expenses", {
        method: "POST",
  ...
  return (
    <div className="mx-auto max-w-2xl space-y-8 p-4 pb-24 pt-6 sm:p-6 md:p-8 lg:pb-8">
      <header className="text-center sm:text-start">
        <span className="mb-3 inline-flex items-center gap-2 rounded-full border border-rose-500/30 bg-rose-500/5 px-4 py-1.5 text-xs font-bold tracking-[0.2em] text-rose-600 dark:text-rose-400">
          <Icon name="receipt_long" filled size="sm" />
          {t("newExpense")}
        </span>
      ...
      {success ? (
        <Card className="border-emerald-500/20 bg-emerald-500/[0.06] text-center" padding="lg">
          <Icon
            ...
            size="lg"
          />
          <h2 className="font-headline text-xl font-bold text-on-surface">{t("expenseSaved")}</h2>
          <p className="mt-2 text-on-surface-variant">{t("expenseSavedHint")}</p>
          <button
            type="button"
        ...
        <Card className="shadow-lg shadow-rose-500/5" padding="lg">
          <form onSubmit={handleSubmit} className="space-y-5">
            ...
            <div>
              <label htmlFor="expenseType" className="mb-2 block text-sm font-bold text-on-surface">
                {t("expenseType")}
              </label>
              <select
                id="expenseType"
